...
public class FilterRowImpl extends RowImpl {

	FilterRowImpl(TableWidgetImpl table, int rowIndex) {
	...
	void filter(List<FilterTemplate<Object>> activeFilters) {
		...
		for (FilterTemplate<Object> filter : activeFilters) {
			Object cellContent = content.values.get(filter.columnIndex);
			content.visible &= filter.applies(cellContent);
		}
	}
}
