/**
 * This file is part of FXL GUI API.
 *  
 * FXL GUI API is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *  
 * FXL GUI API is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *  
 * You should have received a copy of the GNU General Public License
 * along with FXL GUI API.  If not, see <http://www.gnu.org/licenses/>.
 *
 * Copyright (c) 2010 Dangelmayr IT GmbH. All rights reserved.
 */
package co.fxl.gui.input.impl;

import java.util.LinkedList;
import java.util.List;

import co.fxl.gui.input.api.IRTFWidget.IComposite;

class CompositeImpl implements IComposite {

	String token;
	String label;
	List<IComposite> composites = new LinkedList<IComposite>();

	CompositeImpl() {
	}

	CompositeImpl(String label) {
		this.label = label;
	}

	@Override
	public IComposite addComposite(String label) {
		IComposite c = new CompositeImpl(label);
		composites.add(c);
		return c;
	}

	@Override
	public void token(String token) {
		this.token = token;
	}

	@Override
	public String toString() {
		if (token != null) {
			return (label != null ? label : "") + "-> " + token;
		} else {
			StringBuilder b = new StringBuilder(label != null ? label : "");
			for (IComposite c : composites) {
				if (b.length() > 0)
					b.append("\n");
				b.append(c);
			}
			return b.toString();
		}
	}

}
