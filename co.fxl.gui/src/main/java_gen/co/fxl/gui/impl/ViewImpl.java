...
public class ViewImpl extends LazyClickListener {
    ...
    private ICallback<java.lang.Void> oneTimeCallback;
    ...
    protected void decorate(boolean isNew) {
        ...
        if (!isNew) {
            ...
            if ((this.viewList.newListener) != null) {
                removeImage = grid.cell(1, 0).valign().center().width(30).align()
                                  .end().panel().horizontal().addSpace(4).add()
                                  .image();
            ...
            final IClickable.IClickListener acceptListener = new IClickable.IClickListener() {
                    @Override
                    public void onClick() {
                        labelPanel.clear();
                        decorate(false);
                        ...
                        ViewImpl.this.viewList.newListener.onNew(ViewImpl.this,
                            new CallbackTemplate<java.lang.Void>() {
                                @Override
            ...
            final IImage accept = labelPanel.addSpace(4).add().image()
                                            ...
                                            .addClickListener(acceptListener)
                                            .mouseLeft();
            tf.addKeyListener(new IClickable.IClickListener() {
                    @Override
                    public void onClick() {
                        if (accept.clickable()) {
            ...
            tf.addUpdateListener(new IUpdateable.IUpdateListener<java.lang.String>() {
                    @Override
                    public void onUpdate(String value) {
                        accept.clickable(check(value));
                    }
                });
            labelPanel.addSpace(4).add().image().resource(Icons.CANCEL).addClickListener(new IClickable.IClickListener() {
                    @Override
                    public void onClick() {
    ...
    private boolean check(String value) {
        ...
        for (int i = 0; i < (value.length()); i++)
            if (!((java.lang.Character.isLetterOrDigit(value.charAt(i))) ||
                    ((value.charAt(i)) == ' '))) {
                return false;
            }
    ...
    public void onAllowedClick() {
        ...
        decorator.decorate(next, sideContent.next().panel().vertical(),
            new CallbackTemplate<java.lang.Void>(oneTimeCallback) {
                @Override
                public void onSuccess(Void result) {
                    ...
                    if ((oneTimeCallback) != null) {
                        ICallback<java.lang.Void> cb = oneTimeCallback;
    ...
    public void onClick(ICallback<java.lang.Void> cb) {
        oneTimeCallback = cb;
        onClick();
    }
}
